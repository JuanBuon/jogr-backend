...
# ───────────────────────── Helpers ──────────────────────
def oauth_doc(uid):             # /users/{uid}/oauth/strava
...
def ensure_access_token(uid: str) -> str:
    ...
    if time.time() > data["expires_at"] - 300:
        ...
        r.raise_for_status()
        fresh = r.json(); fresh["expires_at"] = time.time() + fresh["expires_in"]
...
def _head(): return PlainTextResponse("", 200)
...
def strava_callback(code: str = Query(None)):
    ...
    if not q:
        db.collection("users").document(uid).set({
            "userID": uid, "stravaID": sid, "nickname": nick,
    ...
    return RedirectResponse(f"jogr://auth?userID={uid}&code={code}", 302)
...
def save_activity(p: dict = Body(...)):
    ...
    if not need.issubset(p): raise HTTPException(400, "Faltan campos")
...
def league_ranking(lid: str):
    ...
    def score(arr):
        dist=sum(a["distance"] for a in arr); time_m=sum(a["duration"] for a in arr)
        elev=sum(a["elevation"] for a in arr); runs=len(arr)
        ...
        s+=min(50,round(time_m/10)); s+=20 if runs>=3 else 0; return s
...
def toggle_like(act: str, uid: str):
    ...
    doc = ref.get(); likes = doc.to_dict().get("users", []) if doc.exists else []
    if uid in likes: likes.remove(uid); did=False
    else: likes.append(uid); did=True
    ref.set({"users": likes})
...
def add_comment(act: str, p: dict = Body(...)):
    ...
    db.collection("activities").document(act).collection("comments").document(cid).set({
        "userID":p["userID"], "nickname":p["nickname"], "text":p["text"],
