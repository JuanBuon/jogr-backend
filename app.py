...
def _fmt_act(d: dict) -> dict:
    """Homogeneiza documento Firestore → JSON para la app."""
    out = {
        "userID"   : d["userID"],
    ...
    # Campos opcionales que ya almacenamos en Firestore
...
# ahora en /auth/strava en vez de en "/"
@app.get("/auth/strava")
def strava_callback(code: str = Query(None)):
    if not code:
        # Si alguien llama sin ?code, devolvemos un 200 sencillo.
        return PlainTextResponse("JogR backend activo, falta código OAuth.", status_code=200)
    ...
    # Creamos o buscamos usuario en Firestore
    ...
    if q:
        ...
        db.collection("users").document(uid).set({
            ...
            "description": "", "platforms": {"strava": sid}
        })

    # Guardamos tokens con fecha de expiración
    tok["expires_at"] = time.time() + tok["expires_in"]
    oauth_doc(uid).set(tok)

    # Redirigimos a la app móvil con el esquema personalizado
...
def save_activity(p: dict = Body(...)):
    ...
    if not need.issubset(p):
        raise HTTPException(400, "Faltan campos al guardar actividad")
...
def add_comment(act: str, p: dict = Body(...)):
    ...
    if not need.issubset(p):
        raise HTTPException(400, "Faltan campos al añadir comentario")
...
def save_achievements(p: dict = Body(...)):
    ...
    locked   = p.get("locked", [])
    if not uid:
        raise HTTPException(400, "Falta userID al guardar logros")
